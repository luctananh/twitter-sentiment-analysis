...
def train_and_evaluate(
    ...
    try:
        if model_type == "linear_svc":
            model = LinearSVC(C=C, max_iter=SVC_MAX_ITER)
        else:
...
def main():
    ...
    print(
        f"\nCấu hình: C={SVC_C} | ngram={TFIDF_NGRAM_RANGE} | min_df={TFIDF_MIN_DF} | lemma={USE_LEMMATIZATION}"
    )
    print("Thực thi thử nghiệm với TF-IDF + LinearSVC\n")

    accuracy, report = train_and_evaluate(tweets)
    if accuracy is not None:
        print(f"  Mô hình: linear_svc")
        print(f"  C: {SVC_C}")
