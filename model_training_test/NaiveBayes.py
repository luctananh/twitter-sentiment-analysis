...
def train_and_evaluate(tweets, vectorizer_type='bow', model_type='naive_bayes', model_path="models/NaiveBayes_model.sav", C=1.0):
    ...
    # Chuyển văn bản thành vector (BoW)
    if vectorizer_type == 'bow':
        vectorizer = CountVectorizer()
        X = vectorizer.fit_transform(processed_texts)
    else:
        raise ValueError('Invalid vectorizer type. Please use "bow"')
...
def load_model(model_path="models/NaiveBayes_model.sav"):
...
def main():
    ...
    for c_value in [1.0]:
        accuracy, report = train_and_evaluate(tweets, vectorizer_type='bow', model_type='naive_bayes', C=c_value)
